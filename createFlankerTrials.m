function out = createFlankerTrials(coherence,practice,trials_per_cell)

trialMat = fullfact([length(coherence) 2 2]); % [2 = left and right length(coherence) = coherence]
trialMat = [trialMat(:,1) ones(length(trialMat),1) trialMat(:,2:3)];
...
if practice == 1
    ...
    trialMat = repmat(trialMat,trials_per_cell/2,1);
